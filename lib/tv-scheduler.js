...
export class ContentLibrary {
  ...
  getStats() {
    return {
      ...
      tags: this.tags.size,
      totalPlays: Array.from(this.content.values()).reduce((sum, c) => sum + c.playCount, 0)
    };
  }
}
...
export class PlayHistory {
   ...
   */
  addEntry(program, startTime, endTime = null) {
    const entry = {
      ...
      viewers: 0 // Would be populated by broadcast system
    };
  ...
  getMostPlayed(count = 10) {
    ...
    this.history.forEach(entry => {
      playCounts[entry.programId] = (playCounts[entry.programId] || 0) + 1;
    });
