...
export class ScheduleOptimizer {
   ...
   */
  simulateSchedule(schedule, viewershipHistory) {
    const simulation = {
      expectedViewers: 0,
    ...
    schedule.forEach(program => {
      const historicalData = viewershipHistory.get(program.id);
      if (historicalData && historicalData.length > 0) {
        const avgViewers = historicalData.reduce((sum, d) => sum + d.viewers, 0) / historicalData.length;
        const avgEngagement = historicalData.reduce((sum, d) => sum + d.engagementScore, 0) / historicalData.length;
        
        simulation.expectedViewers += avgViewers;
        simulation.expectedEngagement += avgEngagement;
      } else {
        // Default estimates for new content
        simulation.expectedViewers += 100;
        simulation.expectedEngagement += 50;
      }
    });
