...
class SchwepeTVScheduler {
    ...
    getVideosForTimeSlot(scheduleData, currentTimeSlot, seed = this.GMT_SEED) {
        ...
        // Calculate current slot time in seconds from start of day
        const slotHour = Math.floor(currentTimeSlot.timeSlot / 2);
        const slotMinute = (currentTimeSlot.timeSlot % 2) * 30;
        const currentSlotSeconds = slotHour * 3600 + slotMinute * 60;
        ...
        // Find all schedule entries that are ACTIVE during this time slot
        // (not just entries that START at this time)
        const matchingEntries = scheduleData.s.filter(entry => {
            if (entry.d !== currentTimeSlot.dayName) return false;

            const entryStartSeconds = this.timeStringToSeconds(entry.t);
            const entryEndSeconds = entryStartSeconds + (entry.du || 1800); // Default 30min

            // Check if current slot overlaps with this entry's time range
            // Slot spans 30 minutes: [currentSlotSeconds, currentSlotSeconds + 1800)
            const slotEndSeconds = currentSlotSeconds + 1800;

            const overlaps = currentSlotSeconds < entryEndSeconds && slotEndSeconds > entryStartSeconds;

            if (overlaps) {
                console.log(`✅ ${timeString} overlaps with ${entry.t} (${entry.du}s)`);
            }
        ...
        if (matchingEntries.length === 0) {
            console.warn(`⚠️ No active schedule entry for ${currentTimeSlot.dayName} ${timeString}`);
            return { videos: [], isMontage: false };
        }

        // If multiple entries overlap this slot, use deterministic selection
